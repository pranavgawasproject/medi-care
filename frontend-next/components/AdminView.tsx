...
import { SPECIALTIES, colorForName, CLINIC_WEEKLY_TREND } from '@/lib/data/seed'
...
export function AdminView({
  ...
  onUpdateStatus: (appt: Appointment, newStatus: AppointmentStatus) => void
}) {
  ...
  const filteredAppts = useMemo(() => {
    ...
    return appointments.filter((a) => {
      const p =
        patients.find((x) => x.id === a.patient_id)?.name?.toLowerCase() ?? ''
      const d =
        doctors.find((x) => x.id === a.doctor_id)?.full_name?.toLowerCase() ?? ''
  ...
  const getDoctorName = (id: string): string =>
    doctors.find((d) => d.id === id)?.full_name ?? 'Unknown Doctor'
  const getDoctorSpecialty = (id: string): string =>
    doctors.find((d) => d.id === id)?.specialization ?? '—'
  ...
  const handleOnboard = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const finalName = name.trim().startsWith('Dr.')
      ? name.trim()
      : `Dr. ${name.trim()}`
    onAddDoctor({
      full_name: finalName,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-xl text-sm text-muted-foreground">
              Oversee practitioners, monitor appointments, and onboard new
              talent. Real-time visibility across the entire clinic network.
            </p>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <AdminStatCard
          icon={Stethoscope}
          label="Onboarded Doctors"
          value={doctors.length.toString()}
          trend={{ dir: 'up', text: 'Active practitioners' }}
        ...
        <AdminStatCard
          ...
          trend={{ dir: 'up', text: 'In the system' }}
          accent="border-l-primary"
        />
        <AdminStatCard
          icon={CalendarCheck}
        ...
        <AdminStatCard
          ...
          label="Avg Wait Time"
          value="8m"
          trend={{ dir: 'down', text: '-3m improvement' }}
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <Card className="lg:col-span-3 overflow-hidden">
          <CardHeader className="bg-muted/30">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>
                  <TrendingUp className="h-4 w-4 text-primary" /> Weekly
                  Bookings Trend
                </CardTitle>
                <CardDescription>
              ...
              <Badge className="bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/25">
                +18.2% WoW
              </Badge>
            </div>
          </CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <CartesianGrid
                    strokeDasharray="3 3"
                    stroke={
                      isDark ? 'oklch(1 0 0 / 0.08)' : 'oklch(0 0 0 / 0.08)'
                    }
                    vertical={false}
                  />
                  <XAxis
        ...
        <Card className="lg:col-span-2 overflow-hidden">
          <CardHeader className="bg-muted/30">
            ...
            <CardDescription>
              Register a new practitioner to the clinic
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="bg-muted/30">
          ...
          <CardDescription>
            {doctors.length} active practitioners across all specialties
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {doctors.map((d, i) => (
              ...
              >
                <InitialsAvatar
                  name={d.full_name}
                  color={colorForName(d.full_name)}
                  size="md"
                />
      ...
      <Card className="overflow-hidden">
        <CardHeader className="bg-muted/30">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle>
                <CalendarCheck className="h-4 w-4 text-primary" /> Clinic
                Appointment Log
              </CardTitle>
              <CardDescription>
                All appointments across every practitioner
              </CardDescription>
            </div>
                  ...
                  <TableRow key={a.id} className="group">
                    <TableCell>
                      <div className="flex items-center gap-2.5">
                        <InitialsAvatar
                          name={getPatientName(a.patient_id)}
                          color={colorForName(getPatientName(a.patient_id))}
                          size="sm"
                        />
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <InitialsAvatar
                          name={getDoctorName(a.doctor_id)}
                          color={colorForName(getDoctorName(a.doctor_id))}
                          size="sm"
                        />
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {getDoctorSpecialty(a.doctor_id)}
                          </p>
                        </div>
                      </div>
                    </TableCell>
...
function AdminStatCard({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className={cn(
        'rounded-md border border-border border-l-4 bg-card p-4 transition-colors hover:border-primary/50',
        accent
      )}
    >
      ...
      <p className="mt-2 font-mono text-2xl font-semibold leading-none tracking-tight">
        {value}
      </p>
