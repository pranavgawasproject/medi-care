...
import { ACTING_DOCTOR_ID, colorForName } from '@/lib/data/seed'
import type { AppointmentStatus } from '@/lib/types'
import type { Appointment, Doctor, Patient, Schedule } from '@/lib/types'
...
export function DoctorView({
  ...
  const slotsPerDay = DOCTOR_SELF.max
  const utilization = Math.min(
    100,
    Math.round((confirmed.length / slotsPerDay) * 100)
  )
    ...
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            <InitialsAvatar
              ...
              color={colorForName(DOCTOR_SELF.name)}
              size="lg"
            />
            <div className="space-y-1.5">
              <div className="inline-flex items-center gap-2 rounded-full bg-primary/15 px-2.5 py-0.5 text-[11px] font-semibold text-primary">
                <span className="h-1.5 w-1.5 rounded-full bg-primary" /> Doctor
                Portal
              </div>
              <h2
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          icon={Clock}
          label="Slots Filled"
          value={`${confirmed.length}/${slotsPerDay}`}
          accent="border-l-primary"
        ...
        <StatCard
          icon={TrendingUp}
          label="Weekly Rating"
          value={DOCTOR_SELF.rating.toFixed(1)}
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <Card className="lg:col-span-2 overflow-hidden">
          <CardHeader className="bg-muted/30">
            ...
            <CardDescription>
              Approve or decline incoming consultation requests
            </CardDescription>
          </CardHeader>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <InitialsAvatar
                          name={getPatientName(r.patient_id)}
                          color={colorForName(getPatientName(r.patient_id))}
                          size="sm"
                        />
                        ...
                        <Button
                          ...
                          onClick={() =>
                            updateStatus(
                              r,
                              'confirmed',
                              `Approved ${getPatientName(r.patient_id)}'s request.`
                            )
                          }
                        >
                        ...
                        <Button
                          ...
                          onClick={() =>
                            updateStatus(
                              r,
                              'cancelled',
                              `Declined ${getPatientName(r.patient_id)}'s request.`
                            )
                          }
                        >
        ...
        <Card className="lg:col-span-3 overflow-hidden">
          <CardHeader className="bg-muted/30">
            <CardTitle>
              <CalendarDays className="h-4 w-4 text-primary" /> My Weekly
              Schedule
            </CardTitle>
            <CardDescription>
              Availability template across the week
            </CardDescription>
          </CardHeader>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {mySchedule.map((s, i) => {
                  const fill = Math.round(
                    (s.available_slots / slotsPerDay) * 100
                  )
                  return (
                    <motion.div
                      key={s.id}
      ...
      <Card className="overflow-hidden">
        <CardHeader className="bg-muted/30">
          <CardTitle>
            <Users className="h-4 w-4 text-primary" /> Consultation Requests
            &amp; Bookings
          </CardTitle>
                          ...
                          <Button
                            ...
                            onClick={() =>
                              updateStatus(
                                a,
                                'cancelled',
                                `Declined ${getPatientName(a.patient_id)}.`
                              )
                            }
                          >
                            Decline
                          ...
                          <Button
                            ...
                            onClick={() =>
                              updateStatus(
                                a,
                                'cancelled',
                                `Marked ${getPatientName(a.patient_id)} visit as completed.`
                              )
                            }
                          >
                            Complete
