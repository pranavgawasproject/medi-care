...
test('calculatePatientEmergencySymptomTriagingScore', () => {
  const res = calculatePatientEmergencySymptomTriagingScore({
    symptomSeverityScale: 8,
    chestPainPresent: true,
    shortnessOfBreathPresent: true,
    feverTemperatureF: 103,
    patientAgeYears: 70
  });
  assert.strictEqual(res.valid, true);
  assert.strictEqual(res.triagePriorityTier, 'EMERGENCY_IMMEDIATE_CARE');
  assert.ok(res.totalTriageScore >= 75);

  const invalid = calculatePatientEmergencySymptomTriagingScore({ symptomSeverityScale: 15 });
  assert.strictEqual(invalid.valid, false);
  assert.strictEqual(invalid.error, 'Symptom severity scale must be a number between 1 and 10');
});

test('calculatePatientInpatientReadmissionRiskScore', () => {
  const highRisk = calculatePatientInpatientReadmissionRiskScore({
    lengthOfStayDays: 8,
    isAcuteEmergencyAdmission: true,
    charlsonComorbidityIndex: 3,
    emergencyVisitsPast6Months: 2
  });
  assert.strictEqual(highRisk.valid, true);
  assert.strictEqual(highRisk.readmissionRiskTier, 'HIGH_READMISSION_RISK');
  assert.strictEqual(highRisk.laceIndexScore, 13);
  assert.strictEqual(highRisk.estimated30DayReadmissionRatePct, 25);
  assert.ok(highRisk.recommendation.includes('HIGH 30-DAY READMISSION RISK'));

  const lowRisk = calculatePatientInpatientReadmissionRiskScore({
    lengthOfStayDays: 1,
    isAcuteEmergencyAdmission: false,
    charlsonComorbidityIndex: 0,
    emergencyVisitsPast6Months: 0
  });
  assert.strictEqual(lowRisk.valid, true);
  assert.strictEqual(lowRisk.readmissionRiskTier, 'LOW_DISCHARGE_RISK');
  assert.strictEqual(lowRisk.laceIndexScore, 1);

  const invalid = calculatePatientInpatientReadmissionRiskScore({ lengthOfStayDays: 0 });
  assert.strictEqual(invalid.valid, false);
  assert.strictEqual(invalid.error, 'Length of stay must be a positive integer');
});

test('calculatePatientPerioperativeMedicationHoldAudit', () => {
  const compliant = calculatePatientPerioperativeMedicationHoldAudit({
    medicationName: 'Warfarin',
    medicationCategory: 'anticoagulant',
    plannedProcedureHoursAway: 120,
    hasHighBleedingRiskProcedure: true
  });
  assert.strictEqual(compliant.valid, true);
  assert.strictEqual(compliant.isHoldDurationCompliant, true);
  assert.strictEqual(compliant.recommendedHoldDays, 5);

  const nonCompliant = calculatePatientPerioperativeMedicationHoldAudit({
    medicationName: 'Warfarin',
    medicationCategory: 'anticoagulant',
    plannedProcedureHoursAway: 48,
    hasHighBleedingRiskProcedure: true
  });
  assert.strictEqual(nonCompliant.valid, true);
  assert.strictEqual(nonCompliant.isHoldDurationCompliant, false);
  assert.strictEqual(nonCompliant.actionRequired, 'HOLD_MEDICATION_BEFORE_PROCEDURE');
});

test('calculatePatientChronicCareMonitoringIndex', () => {
  const optimal = calculatePatientChronicCareMonitoringIndex({
    systolicBp: 120,
    diastolicBp: 75,
    fastingGlucoseMgDl: 95,
    adherencePercentage: 95,
    daysSinceLastCheckup: 30
  });
  assert.strictEqual(optimal.valid, true);
  assert.strictEqual(optimal.healthScore, 100);
  assert.strictEqual(optimal.careTier, 'OPTIMAL_CHRONIC_CONTROL');

  const elevated = calculatePatientChronicCareMonitoringIndex({
    systolicBp: 150,
    diastolicBp: 95,
    fastingGlucoseMgDl: 140,
    adherencePercentage: 70,
    daysSinceLastCheckup: 120
  });
  assert.strictEqual(elevated.valid, true);
  assert.strictEqual(elevated.careTier, 'ELEVATED_CHRONIC_RISK');

  const invalid = calculatePatientChronicCareMonitoringIndex({ systolicBp: -1 });
  assert.strictEqual(invalid.valid, false);
});

test('calculatePatientTelehealthTriageScore', () => {
  const erRedirect = calculatePatientTelehealthTriageScore({
    symptomAcuityRating: 8,
    hasChestPain: true,
    patientAgeYears: 60
  });
  assert.strictEqual(erRedirect.valid, true);
  assert.strictEqual(erRedirect.triageTier, 'IMMEDIATE_ER_REDIRECT');
  assert.strictEqual(erRedirect.isEmergencyRedirectNeeded, true);

  const routine = calculatePatientTelehealthTriageScore({
    symptomAcuityRating: 3,
    hasChestPain: false,
    hasShortnessOfBreath: false,
    patientAgeYears: 30
  });
  assert.strictEqual(routine.valid, true);
  assert.strictEqual(routine.triageTier, 'ROUTINE_TELEHEALTH');
...
test('calculatePatientPrescriptionRefillAlertStatus', () => {
  const normal = calculatePatientPrescriptionRefillAlertStatus({
    currentDosesRemaining: 20,
    dailyDoseFrequency: 2,
    pharmacyProcessingDays: 3
  });
  assert.strictEqual(normal.valid, true);
  assert.strictEqual(normal.daysSupplyRemaining, 10);
  assert.strictEqual(normal.isRefillAlertNeeded, false);
  assert.strictEqual(normal.alertUrgencyTier, 'NORMAL_SUPPLY');

  const critical = calculatePatientPrescriptionRefillAlertStatus({
    currentDosesRemaining: 2,
    dailyDoseFrequency: 2
  });
  assert.strictEqual(critical.valid, true);
  assert.strictEqual(critical.daysSupplyRemaining, 1);
  assert.strictEqual(critical.isRefillAlertNeeded, true);
  assert.strictEqual(critical.alertUrgencyTier, 'CRITICAL_RUN_OUT_IMMINENT');

  const invalid = calculatePatientPrescriptionRefillAlertStatus({ currentDosesRemaining: -1 });
  assert.strictEqual(invalid.valid, false);
  assert.strictEqual(invalid.error, 'Current doses remaining must be a non-negative number');
});

test('calculatePatientChronicDiseaseAdherenceScore', () => {
  const normal = calculatePatientChronicDiseaseAdherenceScore({
    takenDosesCount: 28,
    prescribedDosesCount: 30,
    missedDoseStreakDays: 0
  });
  assert.strictEqual(normal.valid, true);
  assert.strictEqual(normal.adherencePct, 93.3);
  assert.strictEqual(normal.adherenceTier, 'OPTIMAL_ADHERENCE');
  assert.ok(normal.recommendation.includes('Optimal medication adherence achieved'));

  const highRisk = calculatePatientChronicDiseaseAdherenceScore({
    takenDosesCount: 15,
    prescribedDosesCount: 30,
    missedDoseStreakDays: 4
  });
  assert.strictEqual(highRisk.valid, true);
  assert.strictEqual(highRisk.adherenceTier, 'NON_ADHERENT_HIGH_RISK');
  assert.ok(highRisk.recommendation.includes('High non-adherence risk'));
...
test('calculateMedicationRefillAdherenceAndReminderScore', () => {
  const normal = calculateMedicationRefillAdherenceAndReminderScore({
    totalPrescribedRefillsCount: 10,
    onTimeRefillsCount: 9,
    currentPillsRemaining: 20,
    dailyDoseFrequency: 2
  });
  assert.strictEqual(normal.valid, true);
  assert.strictEqual(normal.adherenceRatePct, 90);
  assert.strictEqual(normal.daysSupplyRemaining, 10);
  assert.strictEqual(normal.adherenceTier, 'HIGH_ADHERENCE');
  assert.strictEqual(normal.alertLevel, 'NORMAL');

  const urgent = calculateMedicationRefillAdherenceAndReminderScore({
    totalPrescribedRefillsCount: 10,
    onTimeRefillsCount: 6,
    currentPillsRemaining: 2,
    dailyDoseFrequency: 2
  });
  assert.strictEqual(urgent.valid, true);
  assert.strictEqual(urgent.daysSupplyRemaining, 1);
  assert.strictEqual(urgent.isOutofStockRisk, true);
  assert.strictEqual(urgent.alertLevel, 'CRITICAL_REFILL_REQUIRED');

  const invalid = calculateMedicationRefillAdherenceAndReminderScore({ totalPrescribedRefillsCount: -1 });
  assert.strictEqual(invalid.valid, false);
  assert.strictEqual(invalid.error, 'Total prescribed refills count must be a positive number');
});

test('calculatePatientAppointmentNoShowRiskScore', () => {
  const high = calculatePatientAppointmentNoShowRiskScore({
    priorNoShowCount: 3,
    appointmentLeadTimeDays: 25,
    distanceToClinicMiles: 25.0,
    hasReminderSent: false,
    isTelehealth: false
  });
  assert.strictEqual(high.valid, true);
  assert.strictEqual(high.riskTier, 'HIGH_NO_SHOW_RISK');
  assert.ok(high.riskScore >= 65);

  const low = calculatePatientAppointmentNoShowRiskScore({
    priorNoShowCount: 0,
    appointmentLeadTimeDays: 3,
    distanceToClinicMiles: 5.0,
    hasReminderSent: true,
    isTelehealth: true
  });
  assert.strictEqual(low.valid, true);
  const invalid = calculatePatientAppointmentNoShowRiskScore({ priorNoShowCount: -1 });
  assert.strictEqual(invalid.valid, false);
});

test('calculatePatientPolypharmacyDrugInteractionRisk', () => {
  const critical = calculatePatientPolypharmacyDrugInteractionRisk({
    totalActiveMedications: 6,
    highRiskCombinationCount: 2,
    hasRenalImpairment: true,
    hasHepaticImpairment: false,
    patientAge: 70
  });
  assert.strictEqual(critical.valid, true);
  assert.strictEqual(critical.riskTier, 'CRITICAL_INTERACTION_RISK');
  assert.strictEqual(critical.polypharmacyRiskScore, 100);

  const invalid = calculatePatientPolypharmacyDrugInteractionRisk({ totalActiveMedications: -2 });
  assert.strictEqual(invalid.valid, false);
});
